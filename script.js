...
// Persist scroll position across reloads (per page)
(function persistScrollPosition() {
    const storageKey = `scrollPos:${location.pathname}`;
    let saveTimer = null;

    const saveScroll = () => {
        try {
            localStorage.setItem(storageKey, String(window.scrollY || window.pageYOffset || 0));
        } catch (_) {
